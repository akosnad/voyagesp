...
fn gps_init<T, M, const BAUD: u32>(gps_uart: &mut esp_hal::uart::Uart<T, M>, delay: &esp_hal::delay::Delay) -> anyhow::Result<()>
where
    ...
    M: esp_hal::Mode,
{
    ...
    info!("Resetting GPS...");
    gps_uart.write_all(&gps_reset_packet).map_err(|e| anyhow::anyhow!("Failed to write GPS reset packet: {:?}", e))?;
    ...
    gps_uart.write_all(&gps_init_packet).map_err(|e| anyhow!("Failed to write GPS init packet: {:?}", e))?;
    ...
    gps_uart.write_all(&nav5_init_packet).map_err(|e| anyhow!("Failed to write NAV5 init packet: {:?}", e))?;
    ...
    gps_uart.write_all(&gps_rate_init).map_err(|e| anyhow!("Failed to write GPS rate init packet: {:?}", e))?;
    gps_expect_ack(gps_uart, delay).map_err(|_| anyhow!("Failed to receive GPS ACK"))?;
    ...
    gps_uart.write_all(&nav_pvt_rate_init).map_err(|e| anyhow!("Failed to write NAV-PVT rate init packet: {:?}", e))?;
    gps_expect_ack(gps_uart, delay).map_err(|_| anyhow!("Failed to receive GPS ACK"))?;
...
fn gps_expect_ack<T, M>(uart: &mut esp_hal::uart::Uart<T, M>, delay: &esp_hal::delay::Delay) -> anyhow::Result<()>
where
    T: esp_hal::uart::Instance,
    M: esp_hal::Mode,
{
    ...
    loop {
        ...
        loop {
            match it.next() {
                Some(Ok(ublox::PacketRef::AckAck(_))) | Some(Ok(ublox::PacketRef::MgaAck(_))) => return Ok(()),
